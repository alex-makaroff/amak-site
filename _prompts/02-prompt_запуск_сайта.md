...
При решени проблем на сервере можешь вносить правки прямо там, пересобирать, запскать, проверять, кока не добъешься результата.
Делаешь на сервере комит и git push

Но в конце тогда надо будет запулить изменения в локальный проект. 
